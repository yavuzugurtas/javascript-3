...
const evaluate = (() => {
  ...
  evaluate.evaluate = (func, isNative, args) => {
    // this works only for synchronous exercises, care more about this later maybe?
    // especially if it no longer seems important to have console logs show the right file/line
  ...
  evaluate.renderImplementation = (func, log) => {
    ...
    if (log.err) {
      const lineNumber = log.err.coordinates !== undefined
        ? '(' + log.err.coordinates + ')'
        : '';
      console.log(`%c${log.err.name}:`, 'font-weight: bold; color: red', log.err.message, lineNumber);
  ...
  evaluate.errorSearchComponent = (name, err) => {
    ...
    const coordinates = err.coordinates !== undefined
      ? '(' + err.coordinates + ')'
      : '';
    text.innerHTML += '<strong>' + err.name + '</strong>: ' + err.message + ' ' + coordinates;
